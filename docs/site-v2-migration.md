...
Observed implementation baseline at this planning pass: `e73552c8996639c4e4177a0935502f220de2b8e6`. This is a reference, not an attested production release.
...
1. Keep customer Site types and the adapter over Project without renaming global graph tables. `Project` remains the physical owned Product/Site root.
2. Adapt existing `SiteOutcome` rather than introduce Monitor/Task/Objective. Its current identity/inference/page semantics are retained; expectation, criticality, environment, enabled state, execution bindings, assessment/freshness and Flag attribution are additive.
3. Store page/action associations to Outcomes as many-to-many, scoped to the customer Site. Preserve inference source and user corrections separately.
4. Add versioned `OutcomeBinding`, idempotent tenant-scoped `RunRequest`, and evidence-backed `OutcomeAssessment`. Existing `Audit` remains the physical run ledger and is linked rather than renamed.
5. Normalize evidence and coverage by behavior, page, viewport, source and timestamp. Keep attempts separate from successful verification. Cadence, freshness, exclusions and missing context are explicit.
6. Establish durable Flag identity over repeated occurrences with safe matching. Reuse Improvement history where compatible; no global merge based solely on URL/title. Preserve source evidence and verification receipts.
7. Preserve provisional anonymous Site ownership and idempotent account claim. Same hostname is not identity or authorization. Handle repeat imports and native Shopify linking explicitly.
8. Preserve connection ownership and history events. ShopifyShop has its own domain identity; host detection alone cannot connect it to an arbitrary account's Project.
