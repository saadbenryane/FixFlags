...
| Flag → Fix → update Review proof | Partial | Manual | **Yes** (manual update Reviews use Product Review credits; automated Watch Reviews do not) |
...
3. **Verified outcomes** — a fresh update Review with comparable evidence proves whether an attempted change improved the Product.
...
5. **"The loop closes"** — Product Review → Fix → Verify → Watch, with an honest receipt when evidence is inconclusive.
...
- When comparing demos: script Product Review → fix in an editor → explicit attempt → update Review. Do not compete on "watch the agent click around."
