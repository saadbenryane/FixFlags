...
describe('buildFindingPrompt', () => {
  it('builds a branch-scoped prompt for repo scan findings', () => {
    ...
    assert.match(prompt, /Suggested branch: fixflags\//)
    assert.match(prompt, /Suggested commit: Fix security finding in src\/config\.ts/)
    ...
    assert.match(prompt, /Tool target: generic/)
