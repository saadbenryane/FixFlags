...
export function runSlowReplayChecks(result: SlowReplayResult | null): DeterministicFlag[] {
  ...
  // 30s is the probe's "never appeared" sentinel. Absence belongs to the
  // missing-action Flag, not a second claim that a primary action was slow.
  if (result.timeToCtaMs > 8_000 && result.timeToCtaMs < 30_000) {
    findings.push({
