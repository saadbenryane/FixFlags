...
export async function notifyWatchRegression(parentAuditId: string, childAuditId: string): Promise<void> {
  ...
  try {
    await resend.emails.send({
      ...
      html: `<p>Hi${child.user.name ? ` ${child.user.name}` : ''},</p><p>Your FixFlags product watch found <strong>${regressCount}</strong> new or regressed issue${regressCount === 1 ? '' : 's'} on <strong>${host}</strong>.</p><p><a href="${SITE_URL}/report/${child.id}">Open the re-check report</a></p><p>Cleared: ${summary.fixed.length} · Remaining: ${summary.unchanged.length} · New: ${summary.newIssues.length} · Regressed: ${summary.regressed.length}</p>`,
