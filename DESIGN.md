...
**Progressive / loading:** The living review is editor chrome, not a carded report page. Full-bleed under thin immersive header (`showFooter={false}`), flush Agent | Report split with a divider only (no pane cards), left thinner than right. FixFlags understanding is chat on the left; the Report shell paints immediately on the right and progressively fills its score, priorities, and evidence. Mobile exposes only Agent and Report. Anonymous reports keep public-safe evidence visible and use contextual sign-in only for gated capabilities.
