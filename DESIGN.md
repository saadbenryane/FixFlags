...
Report information architecture is defined once in [`knowledge/report-contract.md`](./knowledge/report-contract.md). The canonical report is one calm, dense-enough workspace: identity, release score and unresolved Flag overview, then a ranked master/detail Fix list containing every unresolved Flag. Keep the list independently scrollable on desktop and place the selected detail after the list on mobile. Legacy details URLs redirect to this workspace.
