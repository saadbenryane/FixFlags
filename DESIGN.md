...
| Product page   | `/products/[id]` Product understanding       | Contract, verified memory, and launch gates live with Made with, Watch, and Signals. The report has no Review context drawer.                             |
...
**Made with:** Technology evidence lives on the signed-in Product detail page, based on that Product’s latest completed Review. Product Contract, verified Product Memory, and launch gates share that page. The summary shows at most four detected technologies; expansion groups the stack and exposes short sanitized evidence labels. Technology confidence is “Verified” or “Strong signal,” never a vendor score. Empty, legacy, partial, unavailable, and same-detector update-review diff states are explicit. Use locally bundled brand marks when available and Lucide category icons as the fallback; never depend on remote logo delivery.
...
Each issue uses one prompt row with an expandable `Fix Prompt` and a branded copy action on the right, then one desktop | mobile evidence pair.
