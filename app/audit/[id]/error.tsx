...
export default function AuditPageError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error(
      JSON.stringify({ level: 'error', event: 'ui.audit.error', digest: error.digest, message: error.message })
    )
  }, [error.message])
  ...
  return (
    <AuditShell>
      <Container variant="report" className="flex flex-1 flex-col items-center justify-center py-24 text-center">
        <Heading as="h1">Could not load audit</Heading>
