...
const startMonitoringAudit = vi.hoisted(() => vi.fn())

vi.mock('@/lib/db', () => ({ prisma: prismaMock }))
vi.mock('@/lib/auth', () => ({ auth: { api: { getSession } } }))
vi.mock('next/headers', () => ({ headers: async () => new Headers() }))
vi.mock('@/lib/audit/monitoring', () => ({ startMonitoringAudit }))
...
describe('POST /api/reports/[id]/monitoring', () => {
  beforeEach(() => {
    ...
    startMonitoringAudit.mockResolvedValue({
      ok: true,
      result: { auditId: 'child-1', status: 'QUEUED' },
    })
  })
  ...
  it('returns 401 when there is no session', async () => {
    ...
    expect(startMonitoringAudit).not.toHaveBeenCalled()
  })
  ...
  it('starts a re-check without consulting quota (never gated)', async () => {
    ...
    expect(body).toEqual({ reportId: 'child-1', status: 'QUEUED' })
    expect(startMonitoringAudit).toHaveBeenCalledWith('parent-1', expect.objectContaining({ id: 'u1' }))
  })
  ...
  it('forwards ownership / incomplete parent errors from startMonitoringAudit', async () => {
    startMonitoringAudit.mockResolvedValue({
      ok: false,
      status: 403,
      error: 'You can only re-check your own reports',
      code: 'FORBIDDEN',
    })
    const res = await POST(postReq(), { params: Promise.resolve({ id: 'parent-1' }) })
