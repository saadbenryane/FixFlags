...
2. Complete ranked Flag browser (`#report-flags`): live report and homepage Report mode are detail-first (full-width Flag detail with prev/next and `N of M`); Product Your priorities keeps the ranked list beside detail. Coverage stays a muted sentence on the Product list (and path labels on Flag rows). Ranking remains Critical-first.
