# Growth

**Canonical home for growth strategy and measurable experiments.** Vision and strategy: [strategy.md](./strategy.md), [offering.md](./offering.md).

## Growth loop

FixFlags grows by improving certainty and recurrence:
- **Awareness:** product review appears in search, integrations, and founder communities.
- **Activation:** first URL review lands with one clear finding.
- **Verification:** update review proves the first fix.
- **Expansion:** Deep Review and Watch convert to paid retention.

## Channel strategy

- Product-led SEO pages for common AI-builder failures.
- Integration content for Lovable, Cursor, Bolt, Replit, and Devin.
- Founder-led launches on Product Hunt and startup communities.
- Creator-native examples and short before/after snippets.
- Referral loops through agency and freelancer client work.

## Pricing and conversion (current)

- Free: 3 product reviews + 1 deep review teaser.
- Pro/Studio pricing and credit cadence are canonical in `lib/billing/plans.ts`.
- Conversion target: upgrade after first successful update review.

## Growth metrics

- Review completion rate of first run.
- Fix prompt acceptance rate.
- Update review completion rate.
- Conversion to paid after first deep review.
- Re-check (update review) retention within 30 days.
- Watch onboarding from paid plans.
- Share + invite conversion from sample reports.
- Cost per review and false-positive rate.

## Guardrails

- Do not grow on vanity traffic that cannot run the loop.
- Optimize signal quality before channel spend.
- No price-driven traffic without proof of post-review conversion.
