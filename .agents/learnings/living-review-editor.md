...
- Report mode is three rows in the same pane: `ReportOutcomeBar`, the explorer body, and the collapsed `ReportContextDisclosure`. Before this, the pane was a 16-section document and the fix list, the one thing the report exists for, sat mid-scroll behind a verdict, callouts, a score band, a recheck strip, a polish pass, and a fix header.
...
- `components/report/__tests__/ReportOutcomeBar.test.tsx` and `ReportContextDisclosure.test.tsx` assert the outcome is stated once and that context stays collapsed until an anchor targets it.
