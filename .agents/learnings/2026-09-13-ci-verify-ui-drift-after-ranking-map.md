...
After `validate:full` is green, the verify job still runs `npm run test:e2e`. Run `34728245471` on `5355ca25` passed `validate:full` (audit 0 vulns, worker:build succeeded) then failed 20 e2e tests. Those specs still expect Shopify-era homepage copy (`Know when customers can't buy`, Protect → `/protect`, Shopify store domain) and old report-shell geometry. The live homepage H1 is `Your website, looked after.` Updating those Playwright files is a separate homepage/report e2e refresh, not a `validate:full` gate.
