...
export function SampleReportDashboardMock({
  ...
  const { host, score, checkedAtLabel, flagCount, rubricCounts, rubricScores, issues, selected } =
    preview
    ...
    >
      <div className="min-h-[32rem] sm:min-h-[36rem]">
        ...
        <div className="grid gap-3.5 border-b border-border/30 p-5 sm:grid-cols-3 sm:gap-4 sm:p-6">
          <SummaryCard>
            <div className="flex items-center gap-3.5">
              ...
              <div className="min-w-0">
                <p className="text-xs font-medium text-muted-foreground">{copy.releaseReadiness}</p>
                <p className="mt-1 font-mono text-lg font-semibold tabular-nums leading-none text-foreground">
                  {score == null ? 'N/A' : score}
                  {score != null ? (
                    <span className="text-sm font-medium text-muted-foreground">{copy.scoreDenom}</span>
                  ) : null}
                </p>
              </div>
            </div>
          ...
          <SummaryCard>
            <p className="text-xs font-medium text-muted-foreground">{copy.highImpact}</p>
            <p className="mt-2 font-mono text-3xl font-semibold tabular-nums leading-none text-foreground">
              {flagCount}
            </p>
            <p className="mt-2.5 text-xs font-medium text-destructive">{copy.needsAttention}</p>
          ...
          <SummaryCard>
            <p className="text-xs font-medium text-muted-foreground">{copy.byRubric}</p>
        ...
        <div className="relative grid gap-5 p-5 sm:p-6 lg:grid-cols-[minmax(0,1fr)_minmax(16rem,18.5rem)] lg:gap-6">
          <div className="min-w-0">
            ...
            <p className="mt-4 text-xs font-medium text-muted-foreground">{copy.viewAll(flagCount)}</p>
          </div>
              ...
              <div className="mt-5 space-y-4">
                ...
                <DetailSection title={copy.fixPrompt}>
                  <div className="line-clamp-4 rounded-[var(--radius-control)] bg-muted/55 px-3.5 py-3 text-[0.6875rem] leading-relaxed text-foreground/80">
                    {selected.hasFixPrompt ? selected.fixPrompt : copy.fixPromptFallback}
                  </div>
...
function MockTab({ label, active = false }: { label: string; active?: boolean }) {
  return (
    <span
      className={cn(
