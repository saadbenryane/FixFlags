  ...
  return <section aria-labelledby="products-heading" className={styles.surface}>
    ...
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
      {products.map(product => {
        ...
        const answer = checking ? 'Checking your website' : review?.status === 'FAILED' ? 'Could not complete the check' : product.topAttention?.title ?? (completed ? review?.reportCompleteness === 'FULL' ? '0 Flags' : 'Coverage is incomplete' : 'Ready for a first check')
        return <BoardCard key={product.id}
          name={product.name}
          state={checking ? 'checking' : product.attentionCount ? 'attention' : completed && review?.reportCompleteness === 'FULL' ? 'healthy' : 'unknown'}
          status={checking ? 'Checking' : completed ? review?.reportCompleteness === 'FULL' ? product.attentionCount ? `${product.attentionCount} Flag${product.attentionCount === 1 ? '' : 's'}` : '0 Flags' : 'Coverage incomplete' : 'Not checked yet'}
          answer={answer}
      ...
      <Link href={'/new' as Route} aria-label="Analyze a website URL" className={styles.addCard}>
        ...
        <strong>{products.length ? 'Add a website' : 'Analyze your first website'}</strong>
