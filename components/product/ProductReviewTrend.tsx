...
export function ProductReviewTrend({
  ...
  const chart = (
    ...
    >
      ...
      {completed.length === 0 ? (
        ...
        <p className="absolute inset-x-0 bottom-0 text-2xs text-muted-foreground">
          {dateLabel(completed[0]?.completedAt || completed[0]?.createdAt || '')}
        </p>
      ) : (
        <div className="absolute inset-x-0 bottom-0 flex justify-between text-2xs text-muted-foreground">
          ...
          <span>
            {dateLabel(completed.at(-1)?.completedAt || completed.at(-1)?.createdAt || '')}
          </span>
        </div>
      )}
    </div>
  ...
  if (embedded) {
    return <div className={cn('overflow-x-auto', className)}>{chart}</div>
  }
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 overflow-x-auto pb-1">{chart}</div>
    </section>
  )
}
