...
export function ImprovementReceipt({
  ...
  const verificationReviewId =
    evidence?.afterAuditId ?? attempt.verificationReviewId
  const sourceHref = sourceFlagId
    ? `/report/${sourceReviewId}?view=report&flag=${encodeURIComponent(sourceFlagId)}#report-flags`
    : `/report/${sourceReviewId}?view=report`
  ...
  return (
    <article className="rounded-nested-md bg-muted/35 p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <Badge
          variant={attempt.outcome === 'IMPROVED' ? 'outline' : 'secondary'}
        >
          {attempt.builder}
        </Badge>
      </div>
        ...
        <div>
          ...
          <dd className="mt-1 text-muted-foreground">
            {attempt.changeSummary ||
              'A builder marked this Improvement ready to verify.'}
          </dd>
        </div>
        <div>
      ...
      {attempt.outcome ? (
        <div className="mt-3 space-y-2 rounded-nested-sm bg-background/70 p-3 text-xs">
          ...
          <p className="text-muted-foreground">
            {attempt.verificationReason ||
              'The update Review recorded a verification outcome.'}
          </p>
          {coverage ? (
            <ul className="grid gap-1 text-muted-foreground sm:grid-cols-2">
              ...
              <li>
                Comparable evidence:{' '}
                {coverage.evidenceComparable ? 'Yes' : 'No'}
              </li>
              <li>
                Affected page covered:{' '}
                {coverage.relevantPageCovered ? 'Yes' : 'No'}
              </li>
              <li>
                Verifier completed: {coverage.verifierExecuted ? 'Yes' : 'No'}
              </li>
              {coverage.verifierStatus ? (
                <li>
                  Verifier status: {coverage.verifierStatus.toLowerCase()}
                </li>
              ) : null}
              {coverage.pageUrl ? (
                <li className="truncate">Scope: {coverage.pageUrl}</li>
              ) : null}
            </ul>
          ) : null}
          {coverage?.failedModules?.length ? (
            <p className="text-destructive">
              Unavailable evidence: {coverage.failedModules.join(', ')}
            </p>
          ) : null}
        ...
        <p className="mt-3 text-xs text-muted-foreground">
          Run a fresh update Review after the change is live. The builder
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        {verificationReviewId ? (
          <Link
            href={`/report/${verificationReviewId}?view=report` as Route}
            className="inline-flex min-h-11 items-center gap-1.5 text-xs font-medium text-link hover:text-link-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
          >
