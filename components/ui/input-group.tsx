...
export function InputGroup({ className, ...props }: InputGroupProps) {
  return (
    <div
      className={cn(
        'flex flex-col gap-1.5 overflow-hidden rounded-xl p-1.5 glass-surface-elevated shadow-glass sm:rounded-full',
...
export const InputGroupInput = React.forwardRef<HTMLInputElement, InputGroupInputProps>(
  ({ className, ...props }, ref) => (
    <input
      ref={ref}
      className={cn(
        'h-12 min-h-12 min-w-0 flex-1 rounded-lg border-0 bg-transparent px-3.5 py-0 text-base leading-none shadow-none sm:rounded-full',
...
export const InputGroupButton = React.forwardRef<HTMLButtonElement, InputGroupButtonProps>(
  ({ className, ...props }, ref) => (
    <button
      ...
      type="button"
      className={cn(
        'inline-flex h-12 min-h-12 w-full shrink-0 items-center justify-center gap-2 rounded-lg px-5 text-base font-semibold sm:rounded-full',
