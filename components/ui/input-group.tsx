...
export function InputGroup({ className, ...props }: InputGroupProps) {
  return (
    <div
      className={cn(
        'flex flex-col gap-1.5 overflow-hidden rounded-full p-1.5 glass-surface shadow-glass',
...
export const InputGroupInput = React.forwardRef<HTMLInputElement, InputGroupInputProps>(
  ({ className, ...props }, ref) => (
    <input
      ref={ref}
      className={cn(
        'h-11 min-w-0 flex-1 rounded-full border-0 bg-transparent px-3.5 py-0 text-base shadow-none',
...
export const InputGroupButton = React.forwardRef<HTMLButtonElement, InputGroupButtonProps>(
  ({ className, ...props }, ref) => (
    <button
      ...
      type="button"
      className={cn(
        'inline-flex h-11 w-full shrink-0 items-center justify-center gap-2 rounded-full px-5 text-base font-semibold',
